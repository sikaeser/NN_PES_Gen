...
parser.add_argument("--max_norm", default=10.0, type=float, help="max norm for gradient clipping")
...
'''
print("Emean", data_provider.EperA_mean)
print("Estdev", data_provider.EperA_stdev)
print("Fmean", data_provider.FperA_mean)
print("Fstdev", data_provider.FperA_stdev)
print("Dmean", data_provider.DperA_mean)
print("Dstdev", data_provider.DperA_stdev)
quit()
#'''
...
def calculate_errors(val1, val2, weights=1):
    with tf.name_scope("calculate_errors"):
        ...
        loss   = mae #mean absolute error loss, was changed as the training set is not known to be good
    return loss, mse, mae
...
#create projector to visualize atom embeddings
config = projector.ProjectorConfig()
embedding_conf = config.embeddings.add()
embedding_conf.tensor_name = nn.embeddings.name
embedding_conf.metadata_path = 'metadata.tsv'
copyfile('atom_labels.tsv', os.path.join(log_dir, 'metadata.tsv'))
...
with tf.Session() as sess:
    ...
    if checkpoint is not None:
        ...
        step = 0
    nn.restore(sess, "./models/fad_final_a")#load parameters from pretrained nnmodel for TL
